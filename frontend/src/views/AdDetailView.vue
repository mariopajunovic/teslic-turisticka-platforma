...
const { data: oglas, loading, error } = useFetch(() =>
  api.get('oglasi', route.params.slug),
)
const { data: sviOglasi } = useFetch(() => api.list('oglasi'))

const slicni = computed(() => {
  if (!oglas.value || !sviOglasi.value) return []
  return sviOglasi.value
    .filter(
      (o) =>
        o.slug !== oglas.value.slug && o.vrsta?.label === oglas.value.vrsta?.label,
    )
    .slice(0, 3)
})
