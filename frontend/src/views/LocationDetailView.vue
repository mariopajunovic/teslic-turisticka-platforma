...
const { data: lokalitet, loading, error } = useFetch(() =>
  api.get('lokaliteti', route.params.slug),
)
const { data: sviLokaliteti } = useFetch(() => api.list('lokaliteti'))

const slicni = computed(() => {
  if (!lokalitet.value || !sviLokaliteti.value) return []
  return sviLokaliteti.value
    .filter((l) => l.slug !== lokalitet.value.slug)
    .slice(0, 3)
})
