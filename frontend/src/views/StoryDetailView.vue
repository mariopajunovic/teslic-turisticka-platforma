...
const { data: prica, loading, error } = useFetch(() =>
  api.get('price', route.params.slug),
)
const { data: svePrice } = useFetch(() => api.list('price'))

const slicne = computed(() => {
  if (!prica.value || !svePrice.value) return []
  return svePrice.value
    .filter((p) => p.slug !== prica.value.slug)
    .slice(0, 3)
})
