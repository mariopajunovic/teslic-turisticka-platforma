...
const { data: biznis, loading, error } = useFetch(() =>
  api.get('biznisi', route.params.slug),
)
const { data: sviBiznisi } = useFetch(() => api.list('biznisi'))

const slicni = computed(() => {
  if (!biznis.value || !sviBiznisi.value) return []
  return sviBiznisi.value
    .filter(
      (b) =>
        b.slug !== biznis.value.slug &&
        b.kategorija?.label === biznis.value.kategorija?.label,
    )
    .slice(0, 3)
})
